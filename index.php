...
function manejarMonitoreo_checkStatus($module_name, $smarty, $sDirLocalPlantillas)
{
    ...
    $estadoCampania = $oPaloConsola->leerEstadoCampania(
        $estadoCliente['campaigntype'],
        $estadoCliente['campaignid']
    );
    ...
    foreach (array_keys($estadoCliente['activecalls']) as $k) {
        if (!isset($estadoCampania['activecalls'][$k])) {
            $respuesta['activecalls']['remove'][] = array(
                'callid' => $estadoCliente['activecalls'][$k]['callid']
            );
    ...
    foreach (array_keys($estadoCliente['agents']) as $k) {
        if (!isset($estadoCampania['agents'][$k])) {
            $respuesta['agents']['remove'][] = array(
                'agent' => obtenerNombreAgenteDesdeCanal($estadoCliente['agents'][$k]['agentchannel'])
            );
                ...
                switch ($evento['event']) {
                        ...
                        if (esEventoParaCampania($estadoCliente, $evento)) {
                            ...
                            if (in_array($evento['new_status'], array('Failure', 'Abandoned', 'NoAnswer'))) {
                                if (isset($estadoCliente['activecalls'][$callid])) {
                                    restarContadorLlamada(
                                        $estadoCliente['activecalls'][$callid]['callstatus'],
                                        $estadoCliente,
                                        $respuesta
                                    );
                            ...
                            $respuesta['log'][] = formatoLogCampania(array(
                                ...
                                'phone'          => $evento['phone'],
                                'queue'          => $evento['queue'],
                                'agentchannel'   => obtenerNombreAgenteDesdeCanal($sCanalAgente),
                                'duration'       => NULL,
                            ));
                        }
                        break;
                    ...
                    case 'agentlinked':
                        ...
                        if (isset($estadoCliente['activecalls'][$callid])) {
                            ...
                            $respuesta['activecalls']['remove'][] = array(
                                'callid' => $estadoCliente['activecalls'][$callid]['callid']
                            );
                        ...
                        if (isset($estadoCliente['agents'][$sCanalAgente])) {
                            ...
                            $estadoCliente['agents'][$sCanalAgente]['callinfo'] = array(
                                ...
                                'queuestart'     => (is_null($evento['datetime_join']) || $evento['datetime_join'] == '')
                                    ? NULL
                                    : $evento['datetime_join'],
                    ...
                    case 'queuemembership':
                        if (
                            ...
                            $respuesta['agents']['remove'][] = array(
                                'agent' => obtenerNombreAgenteDesdeCanal($estadoCliente['agents'][$sCanalAgente]['agentchannel'])
                            );
...
function limpiarNumeroAgente($agentChannel)
{
    $agentChannel = trim((string)$agentChannel);

    if ($agentChannel == '') {
        return '';
    }

    $partes = explode('/', $agentChannel);
    $numero = trim(end($partes));
    $numero = preg_replace('/[^0-9A-Za-z]/', '', $numero);

    return $numero;
}

function obtenerNombreAgenteDesdeCanal($agentChannel)
{
    $agentNumber = limpiarNumeroAgente($agentChannel);
    return obtenerNombreUsuario($agentNumber);
}

function obtenerNombreUsuario($agentNumber)
{
    global $arrConf;

    $agentNumber = trim((string)$agentNumber);
    ...
    if (isset($arrConf['dsn_conn_database']) && trim($arrConf['dsn_conn_database']) != '') {
        $dsn = $arrConf['dsn_conn_database'];
    } elseif (isset($arrConf['dsn_conn_data']) && trim($arrConf['dsn_conn_data']) != '') {
        $dsn = $arrConf['dsn_conn_data'];
    }

    if ($dsn != '') {
        $oDB = new paloSantoDB($dsn);

        if (is_object($oDB) && $oDB->errMsg == '') {
            $sql = "
                SELECT 
                    CASE 
                        WHEN name IS NULL OR TRIM(name) = '' THEN CONCAT('Agente/', number)
                        ELSE name
            ...
            $result = $oDB->getFirstRowQuery($sql, true, array($agentNumber));

            if (is_array($result) && isset($result['nombre_agente']) && trim($result['nombre_agente']) != '') {
                return trim($result['nombre_agente']);
            }
        }
    }

    return 'Agente/' . $agentNumber;
}
...
function formatoAgente($agent)
{
    ...
    $agentChannel = isset($agent['agentchannel']) ? $agent['agentchannel'] : '';
    $agentNumber = limpiarNumeroAgente($agentChannel);
    $nombreAgente = obtenerNombreUsuario($agentNumber);
...
function formatoLlamadaNoConectada($activecall)
{
    ...
    $sDesde = (!is_null($activecall['queuestart']))
        ? $activecall['queuestart']
        : $activecall['dialstart'];
...
function formatoLogCampania($entradaLog)
{
    ...
    $sMensaje = isset($listaMsg[$entradaLog['new_status']])
        ? $listaMsg[$entradaLog['new_status']]
        : $entradaLog['new_status'];

    foreach ($entradaLog as $k => $v) {
        if ($k == 'duration' && !is_null($v)) {
            $v = sprintf(
                '%02d:%02d:%02d',
                ($v - ($v % 3600)) / 3600,
                (($v - ($v % 60)) / 60) % 60,
