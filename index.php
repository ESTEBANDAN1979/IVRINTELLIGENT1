...
function obtenerNombreUsuario($agentChannel)
{
    global $arrConf;

    // Ejemplo recibido: Agent/2143
    $agentParts = explode('/', $agentChannel);
    $agentNumber = end($agentParts);

    $pDB = obtenerConexionCallCenterDB();
    if (!is_object($pDB)) {
        return $agentChannel;
    }

    $query = "SELECT name FROM agent WHERE number = ? LIMIT 1";
    $result = $pDB->getFirstRowQuery($query, true, array($agentNumber));

    if (is_array($result) && isset($result['name']) && trim($result['name']) != '') {
        return $result['name'];
    }
...
function obtenerConexionCallCenterDB()
{
    global $arrConf;

    /*
     * Primero intenta usar la configuracion normal de Issabel.
     * Dependiendo de la version, el DSN puede estar en distintas claves.
     */
    $posiblesDSN = array();

    if (isset($arrConf['cadena_dsn'])) {
        $posiblesDSN[] = $arrConf['cadena_dsn'];
    }

    if (isset($arrConf['issabel_dsn']['issabel'])) {
        $posiblesDSN[] = $arrConf['issabel_dsn']['issabel'];
    }

    if (isset($arrConf['elastix_dsn']['elastix'])) {
        $posiblesDSN[] = $arrConf['elastix_dsn']['elastix'];
    }

    /*
     * Base del call center. En la mayoria de instalaciones Issabel/Elastix
     * el usuario/clave ya vienen incluidos en el DSN global.
     */
    foreach ($posiblesDSN as $dsn) {
        $dsnCallCenter = preg_replace('/dbname=([^;]+)/', 'dbname=call_center', $dsn);
        $pDB = new paloDB($dsnCallCenter);
    ...
    error_log('No se pudo conectar a la base call_center usando la configuracion global de Issabel');
    return false;
}
...
function formatoAgente($agent)
{
    ...
    switch ($agent['status']) {
        ...
        case 'oncall':
            if (isset($agent['callinfo']) && !is_null($agent['callinfo'])) {
    ...
    $nombreAgente = obtenerNombreUsuario($agent['agentchannel']);

    return array(
        'agent'         =>  $nombreAgente,
        'status'        =>  $sEtiquetaStatus,
        'callnumber'    =>  (isset($agent['callinfo']['callnumber']) && !is_null($agent['callinfo']['callnumber'])) ? $agent['callinfo']['callnumber'] : '-',
        'trunk'         =>  (isset($agent['callinfo']['trunk']) && !is_null($agent['callinfo']['trunk'])) ? $agent['callinfo']['trunk'] : '-',
        'desde'         =>  $sDesde,
    );
}
...
{
    // Conexi󮠡 la base de datos
    $dsn = 'mysql:host=localhost;dbname=call_center;charset=utf8';
    $usuario = 'root';
    $contrase񡠽 'Ecosystemcpn.22';

    try {
        $dbh = new PDO($dsn, $usuario, $contrase񡩻
        
        // Preparar y ejecutar la consulta
        $stmt = $dbh->prepare("SELECT name FROM agent WHERE number = :agentNumber");
        $stmt->bindParam(':agentNumber', $agentNumber, PDO::PARAM_STR);
        $stmt->execute();
...
function formatoAgente($agent)
{
    ...
    // Extraer el número de agente del campo 'agentchannel'
    $agentChannel = $agent['agentchannel']; // Ejemplo: 'Agent/2002'
    $agentParts = explode('/', $agentChannel);
    $agentNumber = end($agentParts); // Obtiene '2002'

    // Obtener el nombre de usuario asociado al número de agente
    $username = obtenerNombreUsuario($agentNumber);
